...
globals [mature-count mast-year time-since-mast dead-total border]
breed [trees tree]
...
to reproduce-trees
   if (mast-year = 1) AND age > 50 [hatch-juveniles 82; this simulates masting, number of seeds produced and age of masting based on Packham et al. 2012. Number of seeds produced and survival rates also based on Packham et al. 2012
    [set age 0
     set tree_size 0
     set color green
     set size 0
...
to kill-trees
  ...
  [set dist-dead distance (min-one-of patches with [dead-count = 1] [distance myself])] ; set the distance to nearest patch where a tree has died
    ...
    if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + 
            ...
            (((dist-dead - 30.20579) / 19.34126) * (-0.24 * spatial-feedback-mortality)) +
            (((1.119015e-17)) * -0.27)
...
to mature-thinning
   if [local-BA] of patch-here >= 97 
   [let local-trees trees in-radius 11.4
    let max-BA local-trees with-max [BA]
    ;print (sum [BA] of local-trees * 25) - (sum [BA] of max-BA * 25)
    ;print (sum [BA] of max-BA * 25)
    let local-trees-sorted sort-by [[BA] of ?1 < [BA] of ?2] local-trees
    foreach local-trees-sorted [
      let local-BA-sum (sum [BA] of trees in-radius 11.4) * 25
      if local-BA-sum >= 97 [print [BA] of self die]
    ]
...
to get-local-density
  ...
  [set pcolor scale-color green (tree-density) 0 30]
end
...
205
296
Magnitude of relationships relative to current conditions
11
0.0
1
...
@#$#@#$#@
<experiments>
  <experiment name="Experiment_1" repetitions="4" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    ...
    <enumeratedValueSet variable="DBH-mortality">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="growth-rate-mortality">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="spatial-feedback-mortality">
      <value value="1"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
