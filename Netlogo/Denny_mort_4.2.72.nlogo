...
globals [mature-count mature-BA mast-year dead-total border]
breed [trees tree]
breed [juveniles juvenile]
trees-own [age BA tree-size-t1 tree-size-t2 growth-rate tree-density dist-dead]
juveniles-own [age tree_size tree-density]
patches-own[dead-count time-dead no-juveniles local-BA]
...
to setup
  ...
  reset-ticks
  set dead-total 0
  ask trees [if local-BA > 75 [ask trees in-radius 11.28 with-min [tree-size-t1]  [set dead-count 1 set time-dead 0 set pcolor red die]]]
...
to reproduce-trees
   if (mast-year = 1) AND age > 50 [hatch-juveniles 82 * (1 - juvenile-mortality); this simulates masting, number of seeds produced and age of masting based on Packham et al. 2012. Number of seeds produced and survival rates also based on Packham et al. 2012
    [set age 0
     set tree_size 0
     set color brown
     set size 0
    set heading random-float 360
    let disperse-distance random-exponential 12 ;this sets the dispersal distance at a mean of 12 metres, based on info from Packham et al. 2012
   ...
   if (mast-year = 0) AND age > 50 [hatch-juveniles random-normal 82 5 * (1 - juvenile-mortality); this simulates masting, number of seeds produced and age of masting based on Packham et al. 2012. Number of seeds produced and survival rates also based on Packham et al. 2012
    [set age 0
     set tree_size 0
     set color brown
     set size 0
...
to kill-trees
  ...
  ;if local-BA > 75 [ask trees in-radius 11.28 with-min [tree-size-t1]  [set dead-count 1 set time-dead 0 set pcolor red die]] ;change this so that instead it kills of all those smaller trees that cause the BA to be >75
...
to grow-juveniles ;simulates seedling growth using equations of Collet et al 2001
    ...
    [ifelse [local-BA] of patch-here < 20
...
to kill-juveniles
 ifelse differential-juvenile-mortality?
     [
        if [local-BA] of patch-here < 20 [die]
...
to juvenile-thinning
    if [no-juveniles] of patch-here >= 3 [ask min-one-of juveniles-here [tree_size] [die]]
end
...
to count-juveniles
  ...
  set pcolor scale-color green (no-juveniles * 100) 0 400 
end

to get-local-BA
  set local-BA sum [BA] of trees in-radius 11.28
end
