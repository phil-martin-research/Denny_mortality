globals [] ; q is the probability that butterfly moves
              ; directly to the highest surrounding patch
patches-own [ elevation used? ]
turtles-own [ start-patch turtles-in-radius turtle-count elevation-here]

to setup
  ca
  ; Assign an elevation to patches and color them by it
 file-open "ElevationData_Ex10.txt"
  ...
  ask patches
  [ set pcolor scale-color green elevation 375 1250
    set used? false
  ] ; End of "ask patches"
  crt 500
  [set size 2
    ;set initial locations of butterflies
    setxy xloc - random 10 yloc - random 10
    pen-down
    set start-patch patch-here
  ]
  ; Initialise the "q" parameter
...
to go ; This is the master schedule
  ...
  if ticks >= 1000 [let final-corridor-width corridor-width
    output-print word "Mean number of butterflies in radius: " clumping
    output-print word "Mean number of butterflies on a patch: " number-of-turtles
    export-plot "Corridor-width"
    word "Corridor-output-for-q-" q
    stop]
end

to move ; The butterfly move procedure, in a turtle context
        ; Decide whether to move to the highest
        ; surrounding patch with probability q
  ifelse move-uphill? = TRUE
  [
  ifelse random-float 1 < q ; Move deterministically uphill
  [uphill elevation] ; Or more randomly
  [move-to one-of neighbors]
  ]
  [ ifelse count neighbors with [elevation > [elevation] of myself] >= 1
    [let target one-of neighbors with [elevation > [elevation] of myself]
    move-to target]
    [move-to one-of neighbors]
  ]
    ;ifelse [elevation 
    ;ask turtles [let patches-elevation patch-here [elevation]]
  set used? true
  set pcolor yellow
...
to-report corridor-width
  ifelse ticks >= 1
  [let patch-count count patches with [used? = true]
  let distance-mean mean [distance start-patch] of turtles
  report patch-count / distance-mean]
  [report 0]
  
end

to-report clumping
  ask turtles [set turtles-in-radius count turtles in-radius 3]
  let mean-in-radius mean [turtles-in-radius] of turtles
  report mean-in-radius
end

to-report number-of-turtles
  ask turtles [set turtle-count count turtles-here]
  let mean-turtle-count mean [turtle-count] of turtles
...
to kill-turtles
  let mortality random-float 1
  if mortality > 0.999 [die]
end
@#$#@#$#@
GRAPHICS-WINDOW
1220
...
"" ""
PENS
"pen-0" 1.0 0 -7500403 true "" "plot corridor-width"
