...
to setup
  ...
  [
    ...
    file-close
    ask patches [
      set pcolor scale-color green elevation 467.2 660.2
    ]
  crt 50
  [set size 2
...
to go ; This is the master schedule
  ...
  [
    ...
    output-print word ("No. of butterflies in radius 3:" butterfly-density)
    stop
    ]
end
...
to-report corridor-width
  let visit-count count patches with [used? = true]
  let mean-distance mean [ distancexy 85 95 ] of turtles
  report visit-count / mean-distance
end

to butterfly-density
  show [count turtles in-radius 3] of turtles
end
@#$#@#$#@
GRAPHICS-WINDOW
723
