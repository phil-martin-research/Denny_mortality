...
globals [mature-count mast-year time-since-mast drought-year dead-total border]
breed [trees tree]
...
to grow-trees ;this increases tree size using equations approximately equivalent to those of Holzwarth et al. 2013
  ;young trees increase in DBH rapidly and this slows in older trees
    set age age + 1
    ifelse drought-year = 1[
    set tree-size-t2 tree-size-t1 + 0.18 - (0.003 * tree-size-t1) ]
    [ 
...
to mature-thinning
   ...
   while [[local-BA] of self > 100]
   [set local-BA ((sum [BA] of trees in-radius 11.38) * 25)
   if [local-BA] of self > 100 
...
;
to drought
  ifelse(random-float 1 < (drought-rate / 10))
  [set drought-year 1]
  [set drought-year 0]
end
; REFERENCES
...
"pen-1" 1.0 0 -2674135 true "" "plot mean [growth-rate] of trees + standard-deviation [growth-rate] of trees"
"pen-2" 1.0 0 -2674135 true "" "plot mean [growth-rate] of trees - standard-deviation [growth-rate] of trees"
