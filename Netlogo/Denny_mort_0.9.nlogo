...
to setup-turtles ;create initial trees
  ...
  [set color green
    ...
    set local-BA sum [BA] of trees in-radius 6.35 ;get BA of trees in a radius of 6.35 cells, approximately 40 m^2
    set size 1.5
...
to grow-trees ;this increases tree size using equations approximately equivalent to those of Holzwarth et al. 2013
  ...
  ask trees [
    ...
    if (age > 200) [set tree-size-t2 tree-size-t1 + random-normal 0.2 0.5] ;simulates tree growth for trees >200 years of age
     ...
     set local-BA sum [BA] of trees in-radius 6.35
  ] 
end
...
to kill-trees
  ...
  ask trees [if local-BA > 3 [ask min-one-of trees in-radius 6.35 [tree-size-t1] [die]]] ;this is meant to limit the number of mature trees to <4 per 20m squared, but isn't working any ideas?
end
...
to kill-juveniles
  ...
  ask juveniles[set local-BA sum [BA] of trees in-radius 6.35]
 ifelse differential-juvenile-mortality?
    [ask juveniles [if local-BA < 0.3 [die]]
