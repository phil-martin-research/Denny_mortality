globals [sapling-count mature-count mature-BA]
breed [trees tree]
breed [seedlings seedling]
trees-own [age BA tree-size-t1 tree-size-t2 disperse-distance growth-rate tree-density2]
seedlings-own [age tree_size disperse-distance tree-density1]
...
to setup-turtles ;create initial trees
  ...
  [set color green
    ...
    set tree-density2 count trees in-radius 6.35 ;count number of trees in a radius of 6.35 cells, approximately 40 m^2
    set size 1.5
    setxy random-xcor random-ycor] ;this locates trees in centre of a random patch
  create-seedlings 100 ;creates 100 seedlings
  [set color brown
    ...
    set size 0.5
    set tree-density1 count trees in-radius 6.35;count number of trees in a radius of 6.35 cells, approximately 40 m^2
...
to reproduce-trees
  ask trees [if (random-float 1 > 0.5) and (age > 20) ;trees >20 years old have a 50% chance of producing seedlings
  [hatch-seedlings random-normal 100 2 ;each tree produces a mean of 100 seedlings with an SD of 2 - may need empirical estimate of number of seedlings produced per mature tree
    [set age 0 ;sets seedling age
     set tree_size 0.02 ;sets seedling size
     set color brown
     set size 0.5
    set heading random-float 360
    set disperse-distance random-exponential 10 ;disperses seedlings 
  ]]]
end
...
to grow-trees ;this increases tree size using equations approximately equivalent to those of Holzwarth et al. 2013
  ...
  ask trees [
    ...
    if (age > 200) [set tree-size-t2 tree-size-t1 + random-normal 0.2 0.5] ;simulates tree growth for trees >200 years of age
     ...
     set tree-density2 count trees in-radius 6.35 ;count number of trees in an area equivalent to 20m squared
  ] 
end
...
to kill-trees
  ...
  ifelse tree-size-t1 > 10
  [if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + (((tree-size-t1 - 33) / 24 )* 0.2) + 
            (((growth-rate - 1.9) / 2.5) * -0.59)))) + 0.014) [die]]  
  [if (random-float 1) > (1 - sapling-mortality) [die]] ;for saplings <10 cm DBH, set mortality rate using slider
  ]
    set sapling-count count trees with [(tree-size-t1 > 5) and (tree-size-t1 < 10)] 
    set mature-count count trees with [tree-size-t1 > 10] 
  ...
  if mature-count > 300 [ask n-of (mature-count - 300) trees with [tree-size-t1 > 10] [die]]
  ;this sets maximum number of saplings as 275, derived from Putman et al. (1989)
  if sapling-count > 275 [ask n-of (sapling-count - 275) trees with [tree-size-t1 < 10] [die]] 
  ask trees[
    ;if tree-density2 > 4 [ask one-of trees in-radius 6.35 [die]] ;this is meant to limit the number of mature trees to <4 per 20m squared, but isn't working any ideas?
  ]
end

to age-seedlings ;age seedlings by 1 year per tick
  ask seedlings [set age age + 1] 
end

to grow-seedlings ;simulates seedling growth using equations of Collet et al 2001
  ask seedlings [
    ifelse tree_size < 5
...
to kill-seedlings
  ...
  ask seedlings [set tree-density1 count trees in-radius 6.35]
end
@#$#@#$#@
GRAPHICS-WINDOW
925
...
"Sapling" 1.0 0 -5509967 true "" "plot sapling-count"
