...
to kill-trees
  ...
  [set dist-dead distance (min-one-of patches with [dead-count = 1] [distance myself])] ; set the distance to nearest patch where a tree has died
    ...
    if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + 
            ...
            (((1.119015e-17)) * -0.27)
                  )))) [set dead-count 1 set time-dead 0 set dead 1]
      ] 
     [
    if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + 
...
to mature-thinning
    if [local-BA] of patch-here >= 75 [ask min-one-of trees-here [BA][set dead 1]]
end
...
to-report mean-BA-dead
ifelse count trees with [dead = 1] > 0
[let dead-BA mean [tree-size-t1] of trees with [dead = 1]
report dead-BA]
[report 0]
end
