...
to setup-turtles ;create initial trees
  ...
  [set color green
    set age random-exponential mean-tree-age ;creates set number of trees
  ...
  create-seedlings 100 ;creates 100 seddlings
  [set color brown
...
to reproduce-trees
  ...
  [hatch-seedlings random-normal 100 2 ;each tree produces a mean of 100 seedlings with an SD of 2
    [set age 0 ;sets seedling age
     set tree_size 0.02 ;sets seedling size
     set color brown
     set size 0.5
    set heading random-float 360
    set disperse-distance random-exponential 10 ;disperses seedlings in   
  ]]]
...
to grow-trees ;this increases trese size using
  ask trees [
    ...
    if (age > 200) [set tree-size-t2 tree-size-t1 + random-normal 0.2 0.5] ;simulates tree growth for trees >200 years of age
     set growth-rate tree-size-t2 - tree-size-t1
     set tree-size-t1 tree-size-t2
     set BA (((tree-size-t1 / 200) ^ 2)  * (3.142))
     set tree-density2 count trees in-radius 6.35
  ] 
end
...
to kill-trees
  ...
  [if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + (((tree-size-t1 - 33) / 24 )* 0.2) + (((growth-rate - 1.9) / 2.5) * -0.59)))) + 0.014) [die]];this is derived from the mortality model in the ms
  ...
  if mature-count > 1000 [ask n-of (mature-count - 1000) trees with [tree-size-t1 > 10] [die]]
  if sapling-count > 275 [ask n-of (sapling-count - 275) trees with [tree-size-t1 < 10] [die]] ;this sets maximum number of saplings as 275, derived from Putman et al. (1989)
  ;if tree-density2 > 4 [ask one-of trees in-radius 3 [die]]
end
...
to kill-seedlings
  ...
  ;ask seedlings [if tree-density1 > 4 [die]]
end
@#$#@#$#@
GRAPHICS-WINDOW
925
