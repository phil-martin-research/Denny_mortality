...
to kill-trees
  ...
  [set dist-dead distance (min-one-of patches with [dead-count = 1] [distance myself])] ; set the distance to nearest patch where a tree has died
    ...
    if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + 
            ...
            (((1.119015e-17)) * -0.27)
                  )))) [set dead-count 1 set time-dead 0 print tree-size-t1 die]
      ] 
     [
    if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + 
...
let dead-BA mean [BA] of trees with [dead = 1]
report dead-BA
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [BA] of trees with [dead = 1]"
