...
to setup-turtles ;create initial trees
  ...
  ask n-of n-trees patches [sprout-trees 1 ;creates number of trees set in slider
  [set color brown
    set age random-exponential  mean-tree-age ;creates trees with ages set in slider
    set tree-size-t1 (-1.6 + (35.2 * ln (1 + (0.00061 * (age ^ 1.8))))) + 10 ;set tree size as derived from Holzwarth et al. 2013
  ...
  create-juveniles random 20 * count trees with [age > 50] ;creates a random mnumber of juveniles for each mature tree (up to 20)
  [set color brown
