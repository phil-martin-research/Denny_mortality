...
to kill-trees
  ...
  ask trees [if local-BA > 75 [ask trees with-min [tree-size-t1] in-radius 6.35 [set dead-count 1 set time-dead 0 set pcolor brown die]]] ;change this so that instead it kills of all those smaller trees that cause the BA to be >20m sq
  if mature-BA > 50 [ask trees with-min [tree-size-t1] [set dead-count 1 set time-dead 0 set pcolor brown die]]
