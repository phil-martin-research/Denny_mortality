...
to grow-trees
  ask trees [
    ...
    [set tree_size tree_size + 0.3 + ((random-float 0.2) - 0.1)] ;simulates tree growth so that young trees grow more quickly than old trees
     ...
     set sapling-count count trees with [(tree_size > 5) and (tree_size < 10)] 
     set mature-count count trees with [tree_size > 10] 
     set mature-BA sum [BA] of trees with [tree_size > 10]    
  ] 
end
...
to kill-trees
  ask trees[
  ifelse tree_size > 5
  [if (random-float 1) > 1 - (1 - exp(- exp(-5.029532 + (((tree_size - 33) / 24 )* 0.2)))) [die]];this is derived from the mortality model in the ms
  [if (random-float 1) > (1 - sapling-mortality) [die]]
  ]
  if mature-count > 300 [ask n-of (mature-count - 300) trees with [tree_size > 10] [die]]
  if sapling-count > 275 [ask n-of (sapling-count - 275) trees with [tree_size < 10] [die]] ;this sets maximum number of saplings as 275, derived from Putman et al. (1989)
end
