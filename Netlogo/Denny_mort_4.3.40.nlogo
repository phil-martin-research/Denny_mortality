...
to grow-trees ;this increases tree size using equations approximately equivalent to those of Holzwarth et al. 2013
  ;young trees increase in DBH rapidly and this slows in older trees
    set age age + 1
    ifelse time-since-drought <= 20[
    set tree-size-t2 tree-size-t1 + ((0.25 - (0.003515 * tree-size-t1))/(1 + (exp(-0.8 * (time-since-drought - 10)))))
    ]
    [ 
    set tree-size-t2 tree-size-t1 + 0.25 - (0.003515 * tree-size-t1)
    ]
     set growth-rate (tree-size-t2 - tree-size-t1) * 10 ;this sets DBH growth rate over 1 year in mm
     set tree-size-t1 tree-size-t2 ;set tree size at t2 as tree size at t1 for next tick
...
to kill-trees
  ...
  [set dist-dead distance (min-one-of patches with [dead-count = 1] [distance myself])] ; set the distance to nearest patch where a tree has died
    ...
    [
    let GR (growth-rate - 2.141075) / 1.929474
    let TS (tree-size-t1 - 33) / 24
    let DD (dist-dead - 20) / 31
    
    if (random-float 1) < 1 - exp( - exp(-5.029532 + (-.59 * GR) + (0.2338 * TS) 
      + (-0.1373 * DD) + (1.119015e-17 * 0.1490)
      ))
    ...
    if (random-float 1) < 1 - exp( - exp(-5.029532 + (-.9388 * GR) + (0.21 * TS) 
...
to grow-juveniles ;simulates seedling growth using equations of Collet et al 2001
    ...
    [set tree_size tree_size + random-normal 0.126 0.05] ;seedling growth rates derived from Ammer, Stimm & Mosandl 2008
    [set tree_size tree_size + random-normal 0.109 0.01]
    ]
    [ let tree_size_1 2
      set breed trees
      set color brown
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [local-canopy] of patches"
