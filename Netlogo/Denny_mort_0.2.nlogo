...
trees-own [age BA tree-size-t1 tree-size-t2 disperse-distance growth-rate]
...
to setup-turtles ;create initial trees
  ...
  create-trees 140
  [set color green
    set age random-exponential 100
    set tree-size-t1 -1.6 + (35.2 * ln (1 + (0.00061 * (age ^ 1.8))))
    set tree-size-t2 0
    set size 1.5
    setxy random-xcor random-ycor] ;this locates trees in centre of a random patch
  create-seedlings 100
  [set color brown
...
to age-trees
  ask trees [set age age + 1 ;increases tree age
    if age > 500 [die]] ;kills trees over 500 years age
end

to grow-trees
  ask trees [
    if age < 100 [set tree-size-t2 tree-size-t1 + random-normal 0.4 0.5]
    if (age > 100) AND (age < 200) [set tree-size-t2 tree-size-t1 + random-normal 0.3 0.5] ;simulates tree growth so that young trees grow more quickly than old trees
    if (age > 200) [set tree-size-t2 tree-size-t1 + random-normal 0.2 0.5] 
...
to kill-trees
  ...
  [if (random-float 1) > 1 - (1 - exp(- exp(-5.029532 + (((tree-size-t1 - 33) / 24 )* 0.2) +(((growth-rate - 1.9) / 2.5) * -0.59)))) [die]];this is derived from the mortality model in the ms
...
"Seedlings/5" 1.0 0 -6459832 true "" "plot count seedlings / 5"
