...
to setup-turtles ;create initial trees
  ...
  create-trees 1000
  [set color green
    set age random-exponential 30
    set tree-size-t1 age * 0.6
    set tree-size-t2 0
    set size 1.5
...
to grow-trees
  ask trees [
    ifelse age < 100
    [set tree-size-t2 tree-size-t1 + random-normal 0.5 0.5]
    [set tree-size-t2 tree-size-t1 + random-normal 0.3 0.5] ;simulates tree growth so that young trees grow more quickly than old trees
