...
to grow-juveniles ;simulates seedling growth using equations of Collet et al 2001
  ask juveniles [
    ...
    if juvenile-count > 8000 [ask n-of (juvenile-count - 8000) juveniles [die]]
  ] 
end

to kill-juveniles
  ask juveniles [if random-float 1 > (1 - juvenile-mortality) [die]]
  ask juveniles [set tree-density1 count trees in-radius 6.35]
end
@#$#@#$#@
GRAPHICS-WINDOW
925
