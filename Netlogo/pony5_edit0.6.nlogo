...
to setup-turtles
  ...
  create-trees 400
  [set color green
    set age random-normal 160 20
    set tree_size age * 0.4
    set size 1.5
    setxy random-xcor random-ycor] ;this locates trees in centre of a random patch
  create-seedlings 10000
  [set color brown
    set age random-float 5
...
to age-trees
  ask trees [set age age + 1 ;increases tree age
    if age > 400 [die]] ;kills trees over 500 years age
end
...
to kill-trees
  ask trees[
  ifelse tree_size > 5
  [if (random-float 1) > (1 - (((tree_size - 33) / 24 ) * 0.02))  [die]];this is derived from the mortality model in the ms
