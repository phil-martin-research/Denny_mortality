...
to grow-trees ;this increases tree size using equations approximately equivalent to those of Holzwarth et al. 2013
  ...
  ask trees [
    ...
    if (age > 200) [set tree-size-t2 tree-size-t1 + random-normal 0.2 0.5] ;simulates tree growth for trees >200 years of age
     ...
     set local-BA (sum [BA] of trees in-radius 6.35) * 25
  ] 
end
...
to kill-trees
  ask trees[
    ...
    if (random-float 1) > 1 - ((1 - exp(- exp(-5.029532 + (((tree-size-t1 - 33) / 24 )* 0.2) + 
            (((growth-rate - 1.9) / 2.5) * -0.59))))) [set pcolor brown die]
  ...
  ask trees [if local-BA > 40 [ask min-one-of trees in-radius 6.35 [tree-size-t1] [die]]] ;this is meant to limit the number of mature trees to <4 per 20m squared, but isn't working any ideas?
