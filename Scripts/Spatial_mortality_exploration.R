...
Un_Sp_DBH<-expand.grid(Species=unique(Mort_RC$Species))
for (y in 1:nrow(Un_Sp_DBH)){
Mort_sub<-subset(Mort_RC,Species==Un_Sp_DBH$Species[y])
YU<-unique(Mort_sub$Year)
print(Un_Sp_DBH$Species[y])
Corr<-NULL
for (i in 1:length(YU)){
  Beech_sub<-subset(Mort_sub,Year==YU[i]&!is.na(Dead_cum2))
  b.cor<-spline.correlog(Beech_sub$Easting, Beech_sub$Northing, Beech_sub$Dead_cum2,xmax = 100, resamp=1000,quiet=T)
  b.cor2<-data.frame(Dist=b.cor$boot$boot.summary$predicted$x[1,],Cor=b.cor$boot$boot.summary$predicted$y[6,],UCI=b.cor$boot$boot.summary$predicted$y[2,],LCI=b.cor$boot$boot.summary$predicted$y[10,],Year=YU[i])
...
Moran_plot2<-Moran_plot1+theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.border = element_rect(size=1.5,colour="black",fill=NA))+ theme(legend.position="none")+xlim(0,100)
...
ggsave(paste("Figures/Dead_correl_",Un_Sp_DBH$Species[y],".png",sep=""),height=6,width=8,units="in",dpi=300)
}
